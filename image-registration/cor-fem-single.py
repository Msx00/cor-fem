...
def main(argv=None):
    ...
    with tee_stdout_stderr(os.path.join(args.outdir, "log.txt"), mode="w"):
        ...
        # Stable deformation export for downstream physical-space pull
        # resampling.  Node numbering is shared by reference_nodes and
        # deformed_nodes; surface_node_ids maps surface vertex order to that
        # common volume-node order.
        np.savez_compressed(
            os.path.join(args.outdir, "deformation_data.npz"),
            reference_nodes=np.asarray(volume_points, dtype=np.float64),
            deformed_nodes=np.asarray(
                result["volume_vertices_final"], dtype=np.float64
            ),
            tetrahedra=np.asarray(tets, dtype=np.int64),
            surface_node_ids=np.asarray(surf_map, dtype=np.int64),
            surface_faces=np.asarray(surface_faces, dtype=np.int64),
            surface_vertices_initial=np.asarray(surface_points, dtype=np.float64),
            surface_vertices_final=np.asarray(
                result["surface_vertices_final"], dtype=np.float64
            ),
        )
        np.save(
            os.path.join(args.outdir, "displacement_nodes.npy"),
            np.asarray(result["volume_vertices_final"], dtype=np.float64)
            - np.asarray(volume_points, dtype=np.float64),
        )
